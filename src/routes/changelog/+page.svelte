  ...
  const buildSections = (container: HTMLElement): ReleaseSection[] => {
    ...
    headings.forEach((heading, index) => {
      ...
      const releaseParagraph = nodes.find(
        (node) =>
          node instanceof HTMLParagraphElement && node.querySelector('a')?.textContent?.trim().toLowerCase() === 'release',
        ...
        <div class="changelog-main">
          <div class="changelog-controls">
            <label class="changelog-search">
              <Search class="size-4" />
              <Input placeholder="Search releases, issues, or keywords" bind:value={query} aria-label="Search changelog" />
            </label>
              ...
              {#each filteredSections as section (section.id)}
                {#snippet badge()}
                  {#if section.releaseUrl}
                    <a href={section.releaseUrl} target="_blank" rel="noopener noreferrer" class="changelog-entry__release">
                      Release
...
<style>
  ...
  .changelog-grid {
    position: absolute;
    inset: 0;
    background-image: linear-gradient(to right, color-mix(in oklab, var(--border) 70%, transparent) 1px, transparent 1px),
